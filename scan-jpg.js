...
async function checkFile(timeStr) {
  const name = `WhatsApp Image ${date} at ${timeStr}.jpg`;
  const url = base + encodeURIComponent(name);
  return new Promise((resolve) => {
    const req = http.request(url, { method: 'HEAD' }, (res) => {
      if (res.statusCode === 200) {
        console.log(`FOUND JPG: ${name}`);
        resolve(name);
      } else {
        resolve(null);
      }
...
async function run() {
  console.log('Generating scan list for JPG...');
  const times = [];
  for (let h = 10; h <= 19; h++) {
    for (let m = 0; m < 60; m++) {
      for (let s = 0; s < 60; s++) {
  ...
  console.log(`Scanning ${times.length} times for JPG...`);
  const batchSize = 400;
  const found = [];
